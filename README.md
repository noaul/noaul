...
Noaul 的核心设计是 Windows 优先。目前 Linux 上支持的组件：

- `cc-switch` - 使用 `cc-switch-cli` 通过 `saladday/cc-switch-cli` 的 release install.sh 安装，需要系统已有 `bash` 和 `curl`
- npm 类组件（`codex`、`claude-code`、`opencode`）- 通过 `npm install -g` 安装，需要系统已有 Node.js
- 虚拟组件（`npm`）- 仅验证命令是否可用
...
```bash
curl -fsSL https://noaul.uov.me/linux | bash -s -- cc-switch
NOAUL_DRY_RUN=1 curl -fsSL https://noaul.uov.me/linux | bash
pwsh -NoProfile -Command "./install.ps1 -NoPrompt -DryRun -Install cc-switch -Platform linux"
```
